...
from get_model import load_model, load_mult_model
...
def setup_attack(model):
  ...
  fmodel = PyTorchModel(load_mult_model(model).eval(), bounds=(0, 1), preprocessing=preprocessing)
...
def main(class_ids, class_ids_paths, many_models=False) -> None:
  ...
  if many_models:
    ...
    for model in models:
      ...
      attack, fmodel = setup_attack(model)
    ...
    attack, fmodel = setup_attack('Resnet50')
    adv_accuracies = {}
    for i in range(len(class_ids)):
      class_dl, class_label, class_index = get_class_data(class_ids[i], class_ids_paths[i], bs=50, transform=False)
      print(f"Generating attacks for class: {class_label} ({i}/{len(class_ids)})")
...
if __name__ == "__main__":
  ...
  main(class_ids, class_ids_paths, many_models=args.many_models)
